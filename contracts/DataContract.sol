...
import "./DataContractLibrary.sol";
...
contract DataContract is DataContractInterface, BaseContractZero {
    DataContractLibrary.Data data;
    ...
    constructor(
        ...
        address _ensAddress) public BaseContractZero(_provider, _contractType, _contractDescription, _ensAddress) {
        // has been done in super constructor anyway oO
        // changeContractState(ContractState.Draft);
    }
    ...
    function addListEntries(bytes32[] keys, bytes32[] values) public auth {
        DataContractLibrary.addListEntries(data, keys, values);
    }
    ...
    function changeConsumerState(address targetMember, ConsumerState newState) public {
        DataContractLibrary.changeConsumerState(targetMember, newState);
    ...
    function changeContractState(ContractState newState) public {
        DataContractLibrary.changeContractState(newState);
    ...
    function init(bytes32 domain, bool allowConsumerInviteIn) public auth {
        rootDomain = domain;
        setAllowConsumerInvite(allowConsumerInviteIn);
    }
    ...
    function moveListEntry(bytes32 key, uint256 index, bytes32[] keys) public auth {
        DataContractLibrary.moveListEntry(data, key, index, keys);
    }
    ...
    function removeListEntry(bytes32 key, uint256 index) public auth {
        DataContractLibrary.removeListEntry(data, key, index);
    }
    ...
    function setEntry(bytes32 key, bytes32 value) public auth {
        DataContractLibrary.setEntry(data, key, value);
    }
    ...
    function setMappingValue(bytes32 mappingHash, bytes32 key, bytes32 value) public auth {
        DataContractLibrary.setMappingValue(data, mappingHash, key, value);
    }
    ...
    function getEntry(bytes32 key) public constant returns(bytes32) {
        return DataContractLibrary.getEntry(data, key);
    }
    ...
    function getListEntryCount(bytes32 key) public constant returns(uint256) {
        return DataContractLibrary.getListEntryCount(data, key);
    }
    ...
    function getListEntry(bytes32 key, uint256 index) public constant returns(bytes32) {
        return DataContractLibrary.getListEntry(data, key, index);
    }
    ...
    function getMappingValue(bytes32 mappingHash, bytes32 key) public constant returns(bytes32) {
        return DataContractLibrary.getMappingValue(data, mappingHash, key);
    }
}
