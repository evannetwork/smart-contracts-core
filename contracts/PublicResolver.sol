...
contract PublicResolver {
    ...
    bytes4 constant CONTENTTYPE_INTERFACE_ID = 0x2e4e74e7;
    ...
    event ContentTypeChanged(bytes32 indexed node, uint8 contentType);
    ...
    function supportsInterface(bytes4 interfaceID) constant returns (bool) {
        return interfaceID == ADDR_INTERFACE_ID ||
               ...
               interfaceID == CONTENTTYPE_INTERFACE_ID ||
    ...
    /**
     * Returns the content type id associated with an ENS node.
     * @param node The ENS node to query.
     * @return The associated content type.
     */
    function contentType(bytes32 node) constant returns (uint8 ret) {
        ret = records[node].contentType;
    }

    /**
     * Sets the content hash and content type and type associated with an ENS node.
     * May only be called by the owner of that node in the ENS registry.
     * Note that this resource type is not standardized, and will likely change
     * in future to a resource type based on multihash.
     * @param node The node to update.
     ...
     */
    function setContentAndContentType(bytes32 node, bytes32 hash, uint8 contentType) only_owner(node) {
